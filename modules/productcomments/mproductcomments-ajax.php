...
include_once(dirname(__FILE__).'/mproductcomments.php');

$productCom = new mproductcomments();
...
if (Tools::getValue('action') && Tools::getValue('id_product_comment') && Context::getContext()->cookie->id_customer)
{
	...
	{
		if (!MProductComment::isAlreadyReport(Tools::getValue('id_product_comment'), Context::getContext()->cookie->id_customer) &&
			MProductComment::reportComment((int)Tools::getValue('id_product_comment'), (int)Context::getContext()->cookie->id_customer))
			die('0');
	}
	else if (Tools::getValue('action') == 'usefulness' && Tools::getValue('value') && Tools::getValue('value'))
	{
		if (!MProductComment::isAlreadyUsefulness(Tools::getValue('id_product_comment'), Context::getContext()->cookie->id_customer) &&
			MProductComment::setCommentUsefulness((int)Tools::getValue('id_product_comment'),
...
else if (Tools::getValue('action') && Tools::getValue('secure_key') == $productCom->secure_key)
{
		...
		if (Context::getContext()->customer->id || (!Context::getContext()->customer->id && $allow_guests))
		{
			...
			$customerComment = MProductComment::getByCustomer((int)($id_product), Context::getContext()->cookie->id_customer, true, (int)$id_guest);

			if (!$customerComment || ($customerComment && (strtotime($customerComment['date_add']) + Configuration::get('M_PRODUCT_COMMENTS_MINIMAL_TIME')) < time()))
			{
				$errors = array();
				$customer_name = false;
				...
				if (!count($errors) && $content)
				{
					...
					{
						$tgrade += $grade['grade'];
						$productCommentCriterion = new MProductCommentCriterion((int)Tools::getValue('id_product_comment_criterion_'.$grade['id']));
					...
					if (!$comment->save())
						...
						Context::getContext()->smarty->assign('confirmation', $productCom->l('Comment posted.').((int)(Configuration::get('M_PRODUCT_COMMENTS_MODERATE')) ? ' '.$productCom->l('Awaiting moderator validation.') : ''));
				...
				$errors[] = sprintf($productCom->l('You should wait %d seconds before posting a new comment.'), Configuration::get('M_PRODUCT_COMMENTS_MINIMAL_TIME'));
		}
}
