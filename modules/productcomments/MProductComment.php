...
class MProductComment extends ObjectModel
{
	public $id;
	...
	public static function getByProduct($id_product, $p = 1, $n = null, $id_customer = null)
	{
		if (!Validate::isUnsignedId($id_product))
			return false;
		$validate = Configuration::get('M_PRODUCT_COMMENTS_MODERATE');
		$sort_direction = Configuration::get('BELVG_COMMENTS_SORT_DIRECTION');
		$p = (int)$p;
		$n = (int)$n;
		if ($p <= 1)
			$p = 1;
		...
		$cache_id = 'MProductComment::getByProduct_'.(int)$id_product.'-'.(int)$p.'-'.(int)$n.'-'.(int)$id_customer.'-'.(bool)$validate;
	...
	{
		$cache_id = 'MProductComment::getByCustomer_'.(int)$id_product.'-'.(int)$id_customer.'-'.(bool)$get_last.'-'.(int)$id_guest;
		if (!Cache::isStored($cache_id))
		{
	...
	{
		/* Get all grades */
		$grades = MProductComment::getGradeByProduct((int)$id_product, (int)$id_lang);
		$total = MProductComment::getGradedCommentNumber((int)$id_product);
	...
	public static function getCommentNumber($id_product)
	{
		if (!Validate::isUnsignedId($id_product))
			return false;
		$validate = (int)Configuration::get('M_PRODUCT_COMMENTS_MODERATE');
		$cache_id = 'MProductComment::getCommentNumber_'.(int)$id_product.'-'.$validate;
		if (!Cache::isStored($cache_id))
		{
	...
	public function validate($validate = '1')
	{
		...
		Hook::exec('actionObjectMProductCommentValidateAfter', array('object' => $this));
		return $success;
	}
	...
	{
		parent::delete();
		MProductComment::deleteGrades($this->id);
		MProductComment::deleteReports($this->id);
		MProductComment::deleteUsefulness($this->id);
	}
