...
class Blocknewsletter extends Module
{
	...
	public function install()
	{
		...
		CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'newsletter` (
 		...
 		Db::getInstance()->execute('
		CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'newsletter_lang` (
			`id` int(6) NOT NULL AUTO_INCREMENT,
			`id_newsletter` INTEGER UNSIGNED NOT NULL,
	...
	public function getContent()
	{
		if (Tools::isSubmit('submitUpdate'))
		{
			...
			$men_voucher = Tools::getValue('NW_VOUCHER_CODE_MEN');
			$women_voucher = Tools::getValue('NW_VOUCHER_CODE_WOMEN');

			if (($men_voucher && !Validate::isDiscountName($men_voucher)))
				$this->_html .= $this->displayError($this->l('The voucher code is invalid.'));
			else
			{
				Configuration::updateValue('NW_VOUCHER_CODE_MEN', pSQL($men_voucher));
				$this->_html .= $this->displayConfirmation($this->l('Settings updated'));
			}

			if (($women_voucher && !Validate::isDiscountName($women_voucher)))
				$this->_html .= $this->displayError($this->l('The voucher code is invalid.'));
			else
			{
				Configuration::updateValue('NW_VOUCHER_CODE_WOMEN', pSQL($women_voucher));
				$this->_html .= $this->displayConfirmation($this->l('Settings updated'));
			}
	...
	public function newsletterRegistration()
	{
		...
		if ($_GET['action'] == '0')
		{
			...
			$email = pSQL($_GET['email']);
			$id_gender = (int)$_GET['id_gender'];
			$postcode = !empty($_GET['postcode']) ? pSQL($_GET['postcode']) : '';
			...
			if (!$this->isRegistered($register_status))
			{
				if (Configuration::get('NW_VERIFICATION_EMAIL'))
				{
					...
					if ($register_status == self::GUEST_NOT_REGISTERED)
						$this->registerGuest($email, false, $id_gender, $postcode);

					if ($register_status == self::CUSTOMER_NOT_REGISTERED)
						$this->registerUserData($email, $id_gender, $postcode);
				...
				{
					if ($this->register($email, $register_status, (int)$_GET['id_gender'], pSQL($_GET['postcode'])))
						$this->valid = $this->l('You have successfully subscribed to this newsletter.');
					else
						return $this->error = $this->l('An error occurred during the subscription process.');

					$code = ($id_gender = 1 ? Configuration::get('NW_VOUCHER_CODE_MEN') : Configuration::get('NW_VOUCHER_CODE_WOMEN'));
	 ...
	 * @param int    $register_status
	 */
	protected function register($email, $register_status, $id_gender = '', $postcode = '')
	{
		if ($register_status == self::GUEST_NOT_REGISTERED)
			return $this->registerGuest($email, null, $id_gender, $postcode);
	...
	protected function registerUserData($email, $id_gender = '', $postcode = ''){
		...
		foreach ($customers as $customer) {
			...
			if(!$this->userOrGestDataExists($id_customer))
				Db::getInstance()->execute('INSERT INTO '._DB_PREFIX_.'newsletter_lang (id_newsletter, id_gender, postcode) VALUES ('.(int)$id_customer.', '.(int)$id_gender.', "'.pSQL($postcode).'")');
			else
				Db::getInstance()->execute('UPDATE `'._DB_PREFIX_.'newsletter_lang` SET id_gender = '.(int)$id_gender.', postcode ="'.pSQL($postcode).'" WHERE id_newsletter ='.(int)$id_customer.'');
		}
	 ...
	 * @return bool
	 */
	protected function registerGuest($email, $active = true, $id_gender = '', $postcode = '')
	{
		...
		if(Db::getInstance()->execute($sql)) {
			...
			$subscriber = $this->getGuestByEmail($email);

			$id_newsletter = $subscriber['id'];

			if(!$this->userOrGestDataExists($id_newsletter))
				$sql = 'INSERT INTO '._DB_PREFIX_.'newsletter_lang (id_newsletter, id_gender, postcode)
					VALUES ('.(int)$id_newsletter.', '.(int)$id_gender.', "'.$postcode.'")';
			else
				$sql = 'UPDATE `'._DB_PREFIX_.'newsletter_lang` SET id_gender = '.(int)$id_gender.', postcode ="'.pSQL($postcode).'" WHERE id_newsletter ='.(int)$id_newsletter;
	...
	public function userOrGestDataExists($id) {

		$result = Db::getInstance()->getRow('SELECT id FROM `'._DB_PREFIX_.'newsletter_lang` WHERE id_newsletter = '.(int)$id);
	...
	/**
	 * Returns a guest data by email
	 *
	 * @param string $email
	 *
	 * @return array with guest data
	 */
	protected function getGuestByEmail($email)
	{
		$sql = 'SELECT * FROM `'._DB_PREFIX_.'newsletter` WHERE email = "'.pSQL($email).'"';
	...
	public function confirmEmail($token)
	{
		...
		if ($email = $this->getGuestEmailByToken($token)){
			...
			$id = $this->getIdGuestByEmail($email);
			$id_gender = $this->getIdGenderById($id);
			$activated = $this->activateGuest($email);

		} else if ($email = $this->getUserEmailByToken($token)) {

			$id = $this->getIdCustomerByEmail($email);
			$id_gender = $this->getIdGenderById($id);
		...
		if(isset($id_gender)){
			$discount = $id_gender == 1 ? Configuration::get('NW_VOUCHER_CODE_MEN') : Configuration::get('NW_VOUCHER_CODE_WOMEN');
		}
		...
		setcookie('newsletter_subscribed', true, time() + (86400 * 60), "/");
	...
	public function getIdCustomerByEmail($email) {
		...
		return Db::getInstance()->getValue('SELECT DISTINCT id_customer FROM `'._DB_PREFIX_.'customer` WHERE email ="'.pSQL($email).'"');
	}

	public function getIdGuestByEmail($email) {

		return Db::getInstance()->getValue('SELECT DISTINCT id FROM `'._DB_PREFIX_.'newsletter` WHERE email ="'.pSQL($email).'"');
	}
	 ...
	 * @param sting $id
	 *
	 * @return int id_gender
	 */


	public function getIdGenderById($id) {

		return Db::getInstance()->getValue('SELECT DISTINCT id_gender FROM `'._DB_PREFIX_.'newsletter_lang` WHERE id_newsletter ="'.(int)$id.'"');
	}
	...
	public function confirmSubscription($email)
	{
		if ($email)
		{
			if ($discount = Configuration::get('NW_VOUCHER_CODE_MEN'))
				$this->sendVoucher($email, $discount);
	...
	{
		return Mail::Send($this->context->language->id, 'newsletter_voucher', Mail::l('Newsletter voucher', $this->context->language->id), array('{discount}' => $code, '{unsubscribe_link}' => $this->prepareUnregisterLink($email)), $email, null, null, null, null, null, dirname(__FILE__).'/mails/', false, $this->context->shop->id);
	}
	...
	{
		return Mail::Send($this->context->language->id, 'newsletter_conf', Mail::l('Newsletter confirmation', $this->context->language->id), array('{unsubscribe_link}' => $this->prepareUnregisterLink($email)), pSQL($email), null, null, null, null, null, dirname(__FILE__).'/mails/', false, $this->context->shop->id);
	}
	...
	protected function _prepareHook($params)
	{
			...
			if (Tools::getValue('action') == 1)
			{
				$this->smarty->assign(
					array(
						'msg' => $this->valid ? $this->valid : $this->error,
					)
				);
			}
	...
	public function getConfigFieldsValues()
	{
		return array(
			...
			'NW_VOUCHER_CODE_MEN' => Tools::getValue('NW_VOUCHER_CODE_MEN', Configuration::get('NW_VOUCHER_CODE_MEN')),
			'NW_VOUCHER_CODE_WOMEN' => Tools::getValue('NW_VOUCHER_CODE_WOMEN', Configuration::get('NW_VOUCHER_CODE_WOMEN')),
